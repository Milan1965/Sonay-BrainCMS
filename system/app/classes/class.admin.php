	...
	*/
	Class Admin
	{
		...
		public static function UpdateUser()
		{
			...
			if (isset($_POST['update'])) 
			{
				...
				else
				{
					...
					$upateUser->bindParam(':rank', $_POST['rank']); 
					$upateUser->bindParam(':rank_site', $_POST['rank_site']); 
					$upateUser->bindParam(':eventteam', $_POST['eventteam']); 
					$upateUser->bindParam(':spamteam', $_POST['spamteam']); 
					$upateUser->bindParam(':bouwteam', $_POST['bouwteam']); 
					$upateUser->bindParam(':djteam', $_POST['djteam']); 
					$upateUser->bindParam(':pixelteam', $_POST['pixelteam']); 
					$upateUser->execute(); 
					Admin::succeed("The user is saved!");
				}
			}
		...
		public static function UpdateUserOfTheWeek()
		{
			...
			if (isset($_POST['update'])) 
			{
				$getUserData = $dbh->prepare("SELECT id,username,look FROM users WHERE username = :name");
				$getUserData->bindParam(':name', $_POST['naam']); 
				$getUserData->execute(); 
				$upateUser2 = $getUserData->fetch();
				if ($upateUser = $dbh->prepare("UPDATE uotw SET userid=:userdata,userid2=:userdata2,userid3=:userdata3"))
				{
					$upateUser->bindParam(':userdata', $upateUser2['id']); 
					$upateUser->bindParam(':userdata2', $upateUser2['username']); 
					$upateUser->bindParam(':userdata3', $upateUser2['look']); 
					$upateUser->execute();
		...
		public static function LookSollie($variable)
		{
			...
			if (isset($_GET['look'])) 
			{
				$getSollie = $dbh->prepare("SELECT * FROM cms_sollis WHERE id=:look LIMIT 1");
		...
		public static function DeleteSollie()
		{
			...
			if(isset($_GET['delete'])) 
			{ 
				$deleteSollie = $dbh->prepare("DELETE FROM cms_sollis WHERE id=:newsid");
