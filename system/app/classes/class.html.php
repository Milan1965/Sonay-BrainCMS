<?php
	...
	*/
	class Html 
	{
		...
		public static function pageDJ()
		{
			...
			if(isset($_GET['url']))
			{
				...
				{ 
					...
					$fileExists = F ."{$pageDJ}.php";
					if(file_exists(filter($fileExists)))
					{
						$content = in_array($pageHK, $allowed) ? include(F ."/{$pageHK}.php") : '';
					} 
					else 
					{
						include F . "/404.php"; 
				...
				{
					include F . "/djpaneel.php";
				}
			} 
			else 
			{
				include F . "djhome.php";
				header('Location: '.$config['hotelUrl'].'/djpaneel/djhome');
			}
			switch($pageHK)
			{
				case $pageHK:
		...
		{
			echo '<div class="alert alert-warning alert-dismissible"><a><a href="" type="button" class="close" data-dismiss="alert">x</a><strong> '.$errorName.'</strong></div>';
		}
		public static function errorSucces($succesMessage)
		{
			echo '<div class="alert alert-warning alert-dismissible"><a><a href="" type="button" class="close" data-dismiss="alert">x</a><strong> '.$succesMessage.'</strong></div>';
		}
