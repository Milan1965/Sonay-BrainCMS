...
?>
    <title><?= $config['hotelName'] ?>: Bouw Team</title>
</head>
...
<body>
	...
	<div class="container">
            <div class="row">
                <div class="col-12">
                    <div id="title-headline">Bouw Beheerders</div>
                </div>
            </div>
            <div class="row">
<?php
$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM teams WHERE id in (3)  ORDER BY id DESC");
						...
						while ($Ranks = $getRanks->fetch())
							{
								$getMembers = $dbh->prepare("SELECT id,username,motto,look,online FROM users WHERE bouwteam = :ranid");
...
?>
			...
			<div class="row">
                <div class="col-12">
                    <div id="title-headline">Bouw Leden</div>
                </div>
            </div>
            <div class="row">
<?php
