<?php
    ...
    if(file_exists('acc_head_U_happy.txt') && (time() - 43200 < filemtime('acc_head_U_happy.txt')))
    {
        include_once('acc_head_U_happy.txt');       
    }

    $sql = new MySQLi('127.0.0.1', 'root', '', 'zubbo');
    ...
    if($sql->connect_error)
    {
        ...
        if(file_exists('acc_head_U_happy.txt'))
        {
            include_once('acc_head_U_happy.txt');
            exit;
        }
    ...
    $file = fopen('acc_head_U_happy.txt', 'w');
